...
const swrOptions = {
  revalidateOnFocus: false, // ウィンドウフォーカス時に再フェッチしない
  revalidateOnReconnect: false, // ネットワーク再接続時に再フェッチしない（動画フィードなので重要度は低い）
  dedupingInterval: 60000, // 1分間は同じキーのリクエストを重複排除
  keepPreviousData: true, // 新しいデータをロード中も前のデータを表示維持
};
...
export default function VideoFeed({ onUploadRequest }: VideoFeedProps) {
  const {
    data: videos,
    error,
    isLoading,
  ...
  if (videos.length === 0)
    return <div className="text-center text-white">No videos</div>;
