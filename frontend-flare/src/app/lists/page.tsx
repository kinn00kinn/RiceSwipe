...
interface ListData {
  ...
  name: string;
  is_public: boolean;
  // APIからのレスポンスによっては videoCount が含まれない、または構造が異なる可能性があるため柔軟に対応
  videoCount?: number;
...
export default function ListsPage() {
  ...
  const fetchLists = async () => {
    try {
      ...
      if (res.ok) {
        // 修正: as any[] を追加して配列であることをコンパイラに伝える
        const rawData = (await res.json()) as any[];

        // データ整形: APIがカウントを直接返さない場合や、Supabaseの { list_items: [{ count: 123 }] } 形式などを考慮
        const formattedData = rawData.map((item: any) => ({
          ...item,
          videoCount: item.videoCount ?? item.list_items?.[0]?.count ?? 0,
        }));
